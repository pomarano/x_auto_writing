...
| ログの様子 | 原因 | 対処 |
|------------|------|------|
| `Prepare email` / `Send email` が **スキップ**（0秒・丸アイコン） | 今日の下書きが **既にリポジトリにある** | 手動実行なら修正後のワークフローで再 Run。または該当日の `.md` を削除して再実行 |
| `Email skipped (checklist)` の warning | `X_DAILY_EMAIL_ENABLED` が未設定 | **Variables**（Secrets ではない）で `true` を設定 |
| `Send email` は成功（緑・1秒）だが届かない | SMTP は通過、受信側の問題 | 下記「送信成功だが届かない」を参照 |
| `Send email` は実行されたが届かない | Gmail 認証ミス | アプリパスワード・`MAIL_USERNAME` を再確認 |

#### 送信成功だが届かないとき

GitHub の **Send email (Gmail SMTP)** が緑でも、Gmail が「受け付けた」だけで迷惑メールに入ることがあります。

1. **迷惑メール・プロモーション** タブを確認
2. **送信元 Gmail の「送信済み」** を開く（`MAIL_USERNAME` のアカウント）— ここにあれば送信は成功
3. **`NOTIFY_EMAIL`** が普段見ているアドレスと一致するか（Secrets を再確認）
4. 件名 `[内省テック] X下書き` で Gmail 内検索
5. 別アドレス（別ドメイン）を `NOTIFY_EMAIL` にしてテスト

※ 初回 push に `2026-06-03.md` などが含まれていると、同日の自動実行は「スキップ」になります（意図どおり）。
